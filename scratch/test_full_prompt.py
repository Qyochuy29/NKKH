...
PROMPT_SUPER = (
    ...
    "Địt con mẹ mày! Địt mẹ mày! Con mẹ mày! Bố mày, mẹ mày, tuổi lồn sánh vai! "
    "Bố mày nhờn với mày đấy à? Tao đánh chết mẹ mày bây giờ! Láu lồn, mặt lồn, câm mồm! "
    "Cần cặc gì, đầu buồi, lồn buồi, đĩ, chó má, vcl, đm, đéo, biến đi. "
    "Tao đánh vỡ mồm mày bây giờ! Cứu tôi với, đừng đánh nữa, tha cho em!"
)
...
print("=== FULL TRANSCRIPTION WITH PROMPT_SUPER ===")
for seg in segments:
    print(f"[{seg.start:5.2f} - {seg.end:5.2f}] {seg.text}")
    words = getattr(seg, 'words', []) or []
    word_strs = [f"{w.word}({w.start:.2f}-{w.end:.2f})" for w in words]
